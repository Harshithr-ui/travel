...
function buildItems(pool, seg) {
  ...
  const normalizedImages = pool.map(image => {
    if (typeof image === 'string') {
      return { src: image, alt: '', name: '', info: '' };
    }
    return { 
      src: image.src || '', 
      alt: image.alt || '',
...
export default function DomeGallery({
  ...
  const onTileClick = useCallback(
    (e, itemIndex) => {
      ...
      if (openingRef.current) return;
      // Toggle active state for info display
      if (showInfo) {
        setActiveItemIndex(prev => prev === itemIndex ? null : itemIndex);
      } else {
        openItemFromElement(e.currentTarget);
      }
    },
    [openItemFromElement, showInfo]
  );
        ...
        <div className="stage">
          <div ref={sphereRef} className="sphere">
            {items.map((it, i) => (
              ...
              >
                <div
                  className={`item__image${activeItemIndex === i ? ' active' : ''}`}
                  role="button"
                  tabIndex={0}
                ...
                >
                  ...
                  {showInfo && it.name && (
                    <div className="item__info">
                      ...
                      {it.info && <p className="item__desc">{it.info}</p>}
                    </div>
                  )}
                </div>
              </div>
