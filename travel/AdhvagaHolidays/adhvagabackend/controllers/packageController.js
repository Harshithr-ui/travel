...
export const createPackage = async (req, res) => {
  try {
    ...
    const excludes = parseJsonField(req.body.excludes);
    const hotelCategoryPricing = parseJsonField(req.body.hotelCategoryPricing);
    const hotelDetails = parseJsonField(req.body.hotelDetails);
    const cancellationPolicy = parseJsonField(req.body.cancellationPolicy);
    const bookingPolicy = parseJsonField(req.body.bookingPolicy);
    const importantNotes = parseJsonField(req.body.importantNotes);
...
export const updatePackage = async (req, res) => {
  try {
    ...
    // Validate hotel category pricing format
    if (
      updatedData.hotelCategoryPricing &&
      !Array.isArray(updatedData.hotelCategoryPricing)
    ) {
      return res.status(400).json({ message: "Invalid hotel category pricing format" });
    }

    // Validate hotel details format
    if (
      updatedData.hotelDetails &&
      !Array.isArray(updatedData.hotelDetails)
    ) {
      return res.status(400).json({ message: "Invalid hotel details format" });
    }
